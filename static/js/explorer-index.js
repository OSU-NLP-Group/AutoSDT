$(document).ready(function() {
    ...
    function renderTasks(data) {
        ...
        container.innerHTML = ''; // 清空已有的 carousel 内容

        data.forEach((task, idx) => {
            const taskBox = document.createElement('div');
            taskBox.className = 'box m-5';
            taskBox.innerHTML = `
                <div class="content has-text-centered">
                    <div class="sab_example">
                        <div class="text-box" style="overflow-x: scroll; overflow-y: scroll;">
                        ...
                        <div class="text-box" style="overflow-x: scroll; overflow-y: scroll;">
                            <strong>Annotated Program</strong>
                            <pre><code>${stripCodeBlock(task.output)}</code></pre>
                        </div>
                    </div>
                </div>
            `;
    ...
    function stripCodeBlock(text) {
        return text.replace(/```python\s*|```/g, '').trim();
    }
});
