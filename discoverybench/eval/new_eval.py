...
def ask_dimension_question(query, gold_hypo, gold_workflow, gen_hypo,
                           gen_workflow, dataset_meta, llm_used,
                            dimension, dataset_type, use_column_metadata=True):
    dimension_question = ""
    answer = ""
    score = 0.0
    if dimension == "var":
...
def get_sub_hypotheses(query, hypo, workflow, dataset_meta, llm_used, dataset_type, use_column_metadata=True):
    ...
    sub_hypo_json = get_response(client, _prompt, model=llm_used, max_retry=3, verbose=True)  # 增加重试次数和详细日志
...
def match_context_with_gpt(gold_hyp, gold_context, pred_hyp, pred_context, model='gpt-4o'):
    prompt = f"""\
...
def run_eval_gold_vs_gen_NL_subhypo(query, gold_hypo, gold_workflow, gen_hypo, gen_workflow, dataset_meta, llm_used, context_score, dataset_type,use_column_metadata=True):
    ...
    for dimension in ['var', 'rel']:
        question, answer, score = ask_dimension_question(query, gold_hypo, gold_workflow,
                               ...
                               dimension=dimension, dataset_type=dataset_type, use_column_metadata=use_column_metadata)
...
def run_eval_gold_vs_gen_NL_hypo_workflow(query, gold_hypo, gold_workflow, gen_hypo, gen_workflow, dataset_meta, llm_used, dataset_type, use_column_metadata=True):
    ...
    gold_sub_hypo_json = get_sub_hypotheses(query=query,
                                       ...
                                       dataset_meta=dataset_meta, llm_used=llm_used, dataset_type=dataset_type, use_column_metadata=use_column_metadata)
    ...
    for p_id, gen_subh in enumerate(gen_sub_hypo_json['sub_hypo']):
        ...
        for g_id, gold_subh in enumerate(gold_sub_hypo_json['sub_hypo']):
            ...
            # match context
            context_bool = is_matching_context(gold_subh["text"], gold_subh.get("context", ""), gen_subh["text"], gen_subh.get("context", ""), llm_used)
            if context_bool:
                context_score = 1.0
            else:
                context_score = 0.0
    ...
    for p_id, g_id in gen_subh_to_gold_subh.items():
        if g_id >=0:
            ...
            subh_eval_rec = run_eval_gold_vs_gen_NL_subhypo(query, gold_hypo, gold_workflow, gen_hypo, gen_workflow, dataset_meta, llm_used, context_score, dataset_type=dataset_type, use_column_metadata=use_column_metadata)
